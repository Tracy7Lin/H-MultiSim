...
class Model_Replace_Attack(BaseAttack):
            ...
            if client in server.malicious_clients:
                if round_num >= attack_start:
                    if client.id in [c.id for c in server.selected_clients]:
                        print("model_replace_attack: 恶意客户端参与训练...")
                        
                    client.train_malicious_model_replace(
    ...
    def model_replace(self,server, clients, round_num, attack_start,
                    client_gradients,
                    scale= 20):
            ...
            if (client in malicious_set) and (round_num >= attack_start):
                ...
                for idx, (g,base_param) in enumerate(zip(grads,client.gradient)):
                    ...
                    if np.issubdtype(g.dtype, np.floating):
                        new_g = (scale * g).astype(g.dtype, copy=False)
                    else:
                        tmp = (scale * g.astype(np.float64))
                        try:
